...
const db = await mysql.createConnection({
    ...
    port : '3306',// Replace with your DB password
    database: 'Tracker',
    password:'1234'
});
...
app.post('/announce', async(req, res) => {
    console.log('=====================');
    const { ip, port, files, file_hase } = req.body;
        ...
        try {
            ...
            const filePromises = files.map(async(file, index) => {
                ...
                if (fileRows[0].count === 0) {
                    ...
                    return db.execute(
                        'INSERT INTO Files (host_name, file_name, file_hash) VALUES (?, ?, ?)', [host_name, file, file_hase[index]]
                    );
                }
            });
